"""
Cobre abertura/fechamento de caixa (turno de estacionamento assistido) e
os registros de ponto (entrada/saída junto com o caixa, intervalo à
parte) -- ver Caixa/RegistroPonto em app/models.py.
"""
...
def test_abrir_caixa_cria_caixa_aberto_e_ponto_de_entrada(client, db_session):
...
def test_fechar_caixa_calcula_diferenca_e_bate_ponto_de_saida(client, db_session):
    client.post("/gestao/caixa/abrir")
    ...
    assert ponto_saida is not None

    # caixa fechado -- consegue abrir outro em seguida
    resp2 = client.post("/gestao/caixa/abrir")
...
def test_relatorio_ponto_lista_e_filtra_por_tipo(client):
    ...
    client.post("/gestao/caixa/fechar", json={"valor_contado_dinheiro": 0.0})
